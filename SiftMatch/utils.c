...
char* prepend_path(const char* path, const char* file)
{
	...
	sprintf(pathname, n, "%s/%s", path, file);
...
void display_big_img(IplImage* img, char* title)
{
	...
	if (img->width >= 0.90 * scr_width || img->height >= 0.90 * scr_height)
	{
		...
		smallImg = cvCreateImage(cvSize(img->width * scale, img->height * scale),
			img->depth, img->nChannels);
		cvResize(img, smallImg, CV_INTER_AREA);
	}
	else
		smallImg = cvCloneImage(img);

	cvNamedWindow(title, 1);
	cvShowImage(title, smallImg);
